...
function replaceRegistryPaths(inputStr: string): string {
  return inputStr.replace(/(['"])([\s\S]*?)\1/g, (match, quote, content) => {
    if (content.startsWith('@/registry/')) {
      ...
      return `${quote}@/components/anime-ui/${rest}${quote}`;
...
export const index: Record<string, any> = {`;
  ...
  for (const item of uniqueItemsMap.values()) {
    ...
    const filesWithContent = await Promise.all(
      item.files.map(async (file: any) => {
        ...
        const language = file.language || 'typescript'; // Default to typescript

        // Generate target path if not specified
        let targetPath = file.target;
        if (!targetPath || targetPath === '') {
          // Extract the relative path from registry/
          const match = filePath.match(/registry\/(components|lib|hooks)\/(.+)/);
          if (match) {
            const [, type, rest] = match;
            // Convert /index.tsx to .tsx (e.g., block-text/index.tsx -> block-text.tsx)
            const processedRest = rest.replace(/\/index\.(tsx?|jsx?)$/, '.$1');
            if (type === 'components') {
              targetPath = `components/anime-ui/${processedRest}`;
            } else {
              targetPath = `${type}/${processedRest}`;
            }
          } else {
            targetPath = '';
          }
  ...
  "${item.name}": {
    ...
    files: ${JSON.stringify(validFiles, null, 2)},
...
async function buildRegistry() {
  ...
  // 2. Remove everything except registry.json and registry-schema.json
  const entries = await fs.readdir('public/r');
  await Promise.all(
    entries.map(async (entry) => {
      if (entry === 'registry.json' || entry === 'registry-schema.json') return;
      const entryPath = path.join('public/r', entry);
      await fs.rm(entryPath, { recursive: true, force: true });
    }),
  );

  // 3. Read the merged registry.json
  const registryJsonContent = await fs.readFile(REGISTRY_JSON_PATH, 'utf-8');
  const registryData = JSON.parse(registryJsonContent);

  // 4. Generate individual registry files for each item
  await Promise.all(
    registryData.items.map(async (item: any) => {
      // Read file contents if not already present
      const filesWithContent = await Promise.all(
        (item.files || []).map(async (file: any) => {
          const filePath = typeof file === 'string' ? file : file.path;
          const language = file.language || 'typescript'; // Default to typescript
          ...
          if (file.content) {
            const processedContent = 
              file.content.includes('@/registry') || file.content.includes('@workspace/ui/')
                ? replaceRegistryPaths(file.content)
                : file.content;
            return { ...file, language, target: targetPath, content: processedContent };
          }
      ...
      // Filter out null entries and group by language
      const validFiles = filesWithContent.filter((f) => f !== null);
      ...
      await fs.writeFile(
        path.join(process.cwd(), 'public/r', `${item.name}.json`),
